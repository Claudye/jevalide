...
import { Bag } from '../../../src/validation';
...
describe('InputRule', () => {
  ...
  const bag = new Bag(new Local());
  beforeEach(() => {
    inputRule = new InputRule(bag, ['required', 'minlength:8'], {
...
describe('replace', () => {
  it('should replace outgoing rule name with incoming rule name in the items array', () => {
    const inputRule = new InputRule(bag, ['required', 'minlength:8'], {
      required: 'This field is required',
      minlength: 'This field must be at least 8 characters',
    });
...
describe('convertAcoladeGroupToArray', () => {
  it('should return an array of numbers contained in the acolade group', () => {
    const inputRule = new InputRule(bag, [], []);
