...
export class InputRule {
   ...
   * @param {Rule} rule - Optional. The name of the rule to retrieve.
  ...
  private createRule(
    ...
    if (!message) {
      message = this._bag.getMessage(ruleName, local);
    }
    const ruleCallback = this._bag.getRule(ruleName);
  ...
  set(
    rulesParams: RuleNamed,
    messages?: string | string[] | Record<string, string> | null,
    local?: string,
  ) {
    const rules = Array.isArray(rulesParams)
      ? rulesParams.map((r) => r.trim())
      : rulesParams.split('|').map((r) => r.trim());
  ...
  addHook(name: string, callback: InputCallback<InputInterface>) {
    return this;
  }
}
